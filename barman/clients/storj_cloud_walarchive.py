...
import barman
from barman.cloud import CloudInterfaceStorj, configure_logging
...
def main(args=None):
    ...
    try:
        cloud_interface = CloudInterfaceStorj(
            url=config.destination_url,
            access=config.access.encode('utf-8'))
...
def parse_arguments(args=None):
    ...
    parser = argparse.ArgumentParser(
        description='This script can be used in the `archive_command` '
                    ...
                    'Currently Storj is supported.',
        add_help=False
    )
    parser.add_argument(
        'destination_url',
        help='URL of the cloud destination, such as a bucket in Storj network.'
             ' For example: `sj://bucket/path/to/folder`.'
    )
    parser.add_argument(
        'server_name',
